...
export function gerarDadosSeed() {
  ...
  lancamentos.push({
    ...
    valor: '166.80', equivalenteMensal: '13.90', status: 'Pendente', pago: false,
    observacao: 'Assinatura anual — equivale R$13,90/mês',
    parcela: '', vencimento: '2026-08-08', mes: 'Agosto',
    formaPagamento: 'Crédito', cartao: 'Mercado Pago',
  ...
  // 4. MELI MAIS — R$9,90, mensal, dia 21, pagamento automático
  const gId_meli = id('meli')
  lancamentos.push({
    id: gId_meli,
    tipo: 'Despesa', categoria: 'Assinaturas', descricao: 'Meli Mais',
    ...
    pagamentoAutomatico: true, _seedId: 'meli',
  })
  ...
  // 6. EMPRÉSTIMO MERCADO PAGO (2 Parcelas) — R$23,62 × 2
  ...
  // 7. EMPRÉSTIMO MERCADO PAGO (3 Parcelas) — R$60,63 × 3
...
export function migrarLancamento(l) {
  ...
  // vencimento (bug histórico de timezone / edição). Ao carregar, sempre que
  // houver vencimento, o `mes` é realinhado — assim lançamentos do dia 03/09
  // jamais aparecem em agosto.
  ...
  // Garantir que a Amazon antiga (que pode estar como anual, mas sem o campo) ganhe equivalenteMensal
  if (out.descricao === 'Amazon' && out.recorrenciaTipo === 'Anual' && !out.equivalenteMensal) {
    out.equivalenteMensal = '13.90'
  }

  // Garantir remoção do Nubank se a descrição for Amazon, Meli Mais ou Google One
